...
class _TrekkieHomePageState extends State<TrekkieHomePage> {
  ...
  Future<void> _loadShows() async {
    ...
    try {
      ...
      final moviesByTimeline = content['moviesByTimeline'] as Map<String, List<Movie>>;
        ...
        for (final season in show.seasons) {
          for (final episode in season.episodes) {
            ...
            if (episode.isWatched && episode.watchedDate == null) {
              episode.watchedDate = DateTime.now(); // Default date for existing watched episodes
            }
          }
        }
      }
      ...
      for (final movie in movies) {
        ...
        if (movie.isWatched && movie.watchedDate == null) {
          movie.watchedDate = DateTime.now(); // Default date for existing watched movies
        }
      }
  ...
  Widget _buildMoviesView() {
    return DefaultTabController(
      length: _moviesByTimeline.keys.length,
      child: Column(
        children: [
          TabBar(
            isScrollable: true,
            tabs: _moviesByTimeline.keys.map((timeline) => 
              Tab(text: timeline.replaceAll(' (Prime Timeline)', ''))
            ).toList(),
          ),
          Expanded(
            child: TabBarView(
  ...
  Widget _buildStatsView() {
    final allEpisodes = _shows.expand((s) => s.seasons.expand((season) => season.episodes)).toList();
              ...
              child: Column(
                ...
                children: [
                  ...
                  LinearProgressIndicator(
                    value: totalEpisodesCount > 0 ? (watchedEpisodesCount / totalEpisodesCount).toDouble() : 0.0,
                    backgroundColor: Colors.grey.shade300,
                  ),
                  const SizedBox(height: 8),
                  Text('Episodes: $watchedEpisodesCount / $totalEpisodesCount watched'),
                  Text('Movies: $watchedMoviesCount / $totalMoviesCount watched'),
                  Text('$favoriteEpisodesCount favorite episodes, $favoriteMoviesCount favorite movies'),
                  Text('Episodes: ${((watchedEpisodesCount / totalEpisodesCount) * 100).toStringAsFixed(1)}% complete'),
                  Text('Movies: ${totalMoviesCount > 0 ? ((watchedMoviesCount / totalMoviesCount) * 100).toStringAsFixed(1) : 0}% complete'),
                ],
              ),
            ),
          ),
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final episodes = show.seasons.expand((s) => s.episodes).toList();
                  ...
                  child: ListTile(
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        ...
                        Text('$watched / $total episodes (${(progress * 100).toStringAsFixed(1)}%)'),
                      ],
                    ),
                  ),
                );
    ...
    for (final episode in favoriteEpisodes) {
      ...
      allFavorites.add(Card(
        ...
        child: ListTile(
          ...
          subtitle: Text('${show.title} - S${episode.seasonNumber}E${episode.episodeNumber}'),
          trailing: Row(
